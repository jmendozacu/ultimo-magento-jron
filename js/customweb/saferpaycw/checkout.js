...
Customweb.SaferpayCw = Class.create({
	initialize : function(hiddenFieldsUrl, visibleFieldsUrl, serverUrl, javascriptUrl, saveShippingUrl, methodCode)
	{
		...
		// For FireCheckout
			...
			this.formValidation = function(){
				return this.defaultFormValidation() && new Validation('one-step-checkout-form').validate() && checkpayment();
			};
		} else {
	...
	isAuthorization : function(method)
	{
		...
		if (typeof oscPlaceOrder != 'undefined') {
	...
	savePaymentInfoInBrowser : function(validate)
	{
		// Validate forms
//		eval('var result = ' + this.methodCode + 'validatePaymentFormElements();');
//		if (result == false) {
...
//		}
		...
		if(validate !== false && !this.formValidation())  {
			throw 'invalid input';
        }
	...
	{
		if (this.isAuthorization('hidden') || this.isAuthorization('server') || this.isAuthorization('ajax')) {
			try { this.savePaymentInfoInBrowser(false); } catch (e) { return false; }
		}
	...
	magestoreSaveShippingMethod: function(callOriginal, shipping_method_url, update_shipping_payment, update_shipping_review) {
		if (this.isAuthorization('hidden') || this.isAuthorization('server') || this.isAuthorization('ajax')) {
			this.savePaymentInfoInBrowser(false);
		}
