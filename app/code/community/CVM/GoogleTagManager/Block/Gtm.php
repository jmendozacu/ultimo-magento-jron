		...
		foreach ($collection as $order) {
			if ($i == 0) {
				...
				$data = array(
					...
					'voucherValueBrutto' => round($order->getDiscountAmount(),2),
					'voucherValueNetto' => round(($order->getDiscountAmount()) / 1.19,2),
				...
				$data['voucherValueBrutto'] += $order->getDiscountAmount();
				$data['voucherValueNetto'] += round(($order->getDiscountAmount()) / 1.19,2);
