...
class Iksanika_OneStepCheckout_Block_Checkout 
    extends Mage_Checkout_Block_Onepage_Abstract  
{
    ...
    {
        return ($this->hasShippingErrors() || $this->hasBillingErrors() || $this->hasMethodErrors() || $this->hasShipmentErrors()) 
                ? true 
                : false;
    }
    ...
    public function hasShippingErrors()
    {
        ...
        {
            return (count($this->formErrors['shipping_errors']) == 0) ? false : true;
        }
        return true;
    }
    ...
    public function hasBillingErrors()
    {
        if(count($this->formErrors) > 0)   
        {
            ...
            {
                return (count($this->formErrors['billing_errors']) == 0) ? false : true;
            }
            return true;
        }
        return false;
    ...
    {
        return (!empty($this->formErrors['shipping_method'])) ? true : false;
    }
    ...
    public function _handlePostData()
    {
        ...
        }else    
        {
            ...
            if (!empty($billing_data['taxvat'])) 
            {
                ...
                $this->getQuote()->getBillingAddress()->setTaxId($billing_data['taxvat']);
                $this->getQuote()->getBillingAddress()->setVatId($billing_data['taxvat']);
            }
    ...
    {
        return ($this->getQuote()->getItemsSummaryQty() == 0) ? false : true;
    }
