    ...
    if (req.url ~ "{{url_base_regex}}") {
        ...
        if (req.http.Cookie !~ "frontend=" && !req.http.X-Varnish-Esi-Method) {
            if (client.ip ~ crawler_acl ||
                ...
                # it's a real user, make up a new session for them by passing them to the backend
...
sub vcl_hash {
    ...
    if (req.http.Host ~ "b2b" || (req.http.X-Varnish-Esi-Access == "private" &&
...
sub vcl_fetch {
    ...
    if (req.url ~ "{{url_base_regex}}") {
        ...
        # only remove the User-Agent field from this if it exists
