...
class Customweb_SaferpayCw_ExternalcheckoutController extends Mage_Core_Controller_Front_Action
{
	...
	public function loginAction()
	{
		...
		$this->getLayout()->getBlock('content')->append(
			$this->getLayout()->createBlock('saferpaycw/externalCheckout_login')->setContext($context)
		);
		$this->renderLayout();
	}
	...
	public function registerPostAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			if (true !== ($result = Mage::helper('SaferpayCw/externalCheckout')->validateCustomerData($quote, $data, $registerMethod))) {
		...
		if ($this->getRequest()->isPost()) {
			...
			if (!empty($login['username']) && !empty($login['password'])) {
				try {
					...
					// Mage::helper('SaferpayCw')->logException($e); // PA DSS violation: this exception log can disclose customer password
				}
			} else {
	...
	private function getContext()
	{
		if ($this->_context == null) {
			$this->_context = Mage::getModel('saferpaycw/externalCheckoutContext')->loadByQuote($this->getQuote());
		}
