 ...
 * @version 1.3.251
 */
class Customweb_SaferpayCw_Block_Checkout extends Mage_Core_Block_Template {

	protected function _construct(){
		parent::_construct();
		$this->setTemplate('customweb/saferpaycw/checkout.phtml');
	}
	...
	public function getPaymentMethods(){
		...
		foreach ($payments as $paymentCode => $paymentModel) {
			if (preg_match("/saferpaycw/i", $paymentCode)) {
	...
	public function getHiddenFieldsUrl(){
		return Mage::getUrl('SaferpayCw/process/getHiddenFields', array(
			'_secure' => true 
		));
	}

	public function getVisibleFieldsUrl(){
		return Mage::getUrl('SaferpayCw/process/getVisibleFields', array(
			'_secure' => true 
		));
	}

	public function getAuthorizeUrl(){
		return Mage::getUrl('SaferpayCw/process/authorize', array(
			'_secure' => true 
		));
	}

	public function getJavascriptUrl(){
		return Mage::getUrl('SaferpayCw/process/ajax', array(
			'_secure' => true 
		));
	}

	public function getSaveShippingMethodUrl(){
		return Mage::getUrl('SaferpayCw/onepage/saveShippingMethod', array(
			'_secure' => true 
		));
	}
	...
	public function getPreloadUrl(){
		if (version_compare(Mage::getVersion(), '1.8', '>=')) {
			return Mage::getUrl('SaferpayCw/process/preloadOnepage', array(
				'_secure' => true 
			));
		}
		else {
