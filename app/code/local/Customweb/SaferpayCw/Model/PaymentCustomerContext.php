...
class Customweb_SaferpayCw_Model_PaymentCustomerContext implements Customweb_Payment_Authorization_IPaymentCustomerContext
{
	...
	{
		$customerContext = Mage::getModel('saferpaycw/customercontext')->load($this->customerId);
		if ($customerContext->getId() && $customerContext->getContext()) {
			$result = Mage::helper('SaferpayCw')->unserialize($customerContext->getContext());
	...
	protected function updateContextMapInDatabase(array $map)
	{
		...
		if (!$customerContext->getId()) {
			$customerContext = Mage::getModel('saferpaycw/customercontext');
			$customerContext->setId($this->customerId);
		}

		$customerContext->setContext(Mage::helper('SaferpayCw')->serialize($map));
