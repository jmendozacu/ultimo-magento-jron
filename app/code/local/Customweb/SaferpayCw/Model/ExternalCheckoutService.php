 ...
 * @Bean
 */
class Customweb_SaferpayCw_Model_ExternalCheckoutService implements Customweb_Payment_ExternalCheckout_ICheckoutService {
	...
	public function loadContext($contextId, $cache = true){
		return Mage::getModel('saferpaycw/externalCheckoutContext')->load($contextId);
	}
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		if ($quote->isAllowedGuestCheckout() && Mage::getStoreConfig('saferpaycw/general/external_checkout_account_creation') == 'skip_selection') {
			Mage::helper('SaferpayCw/externalCheckout')->validateCustomerData($quote, array(
				'email' => $emailAddress,
		...
		return Customweb_Core_Http_Response::redirect(Mage::getUrl('SaferpayCw/Externalcheckout/login', array(
			'_secure' => true
		)));
	}
	...
	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages){
		...
		$block = Mage::getSingleton('core/layout')->createBlock('saferpaycw/externalCheckout_shippingMethods')->setContext($context);
		return $block->toHtml();
	}
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		foreach ($activePaymentMethods as $paymentMethodCode => $paymentMethod) {
			if (strpos($paymentMethodCode, 'saferpaycw') === 0) {
	...
	public function renderReviewPane(Customweb_Payment_ExternalCheckout_IContext $context, $renderConfirmationFormElements, $errorMessage){
		...
		$update->load('saferpaycw_external_checkout_review');
		$layout->generateXml();
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		$helper = Mage::helper('SaferpayCw');

		$quote = $context->getQuote();
		if ($context->hasBasketChanged($quote, $request)) {
			$context->updateFromQuote($quote);
			$context->save();
			throw new Exception(Mage::helper('SaferpayCw')->__('The shopping cart has been altered!'));
		}
	...
	private function prepareGuestQuote(Customweb_SaferpayCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
	...
	private function prepareNewCustomerQuote(Customweb_SaferpayCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
	...
	private function prepareCustomerQuote(Customweb_SaferpayCw_Model_ExternalCheckoutContext $context){
		$quote = $context->getQuote();
		$billing = $quote->getBillingAddress();
		$shipping = $quote->isVirtual() ? null : $quote->getShippingAddress();
	...
	private function involveNewCustomer(Customweb_SaferpayCw_Model_ExternalCheckoutContext $context){
		$customer = $context->getQuote()->getCustomer();
		Mage::getSingleton('customer/session')->loginById($customer->getId());
	}

	private function refreshContext(Customweb_SaferpayCw_Model_ExternalCheckoutContext $context){
		if (!($context instanceof Customweb_SaferpayCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_SaferpayCw_Model_ExternalCheckoutContext');
		}
		...
		if (!empty($paymentMethod)) {
			...
			$data = array(
				'method' => 'saferpaycw_' . $paymentMethod,
				'checks' => 4 | 1 | 2 | 32 | 128,
			);
		...
		if ($quote->isVirtual()) {
			$context->setShippingMethodName(Mage::helper('SaferpayCw')->__('No shipping method needed.'));
		} else {
		...
		if ($context->getBillingAddress() !== null && $context->getBillingAddress() instanceof Customweb_Payment_Authorization_OrderContext_IAddress) {
			if (!in_array($context->getBillingAddress()->getCountryIsoCode(), $allowCountries)) {
				throw new Exception(Mage::helper('SaferpayCw')->__('It is not possible to checkout in your country.'));
			}
		}
	...
	private function updateUserSessionWithCurrentUser(Customweb_SaferpayCw_Model_ExternalCheckoutContext $context){
		if (!($context instanceof Customweb_SaferpayCw_Model_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('Customweb_SaferpayCw_Model_ExternalCheckoutContext');
		}
		...
		$quote = $context->getQuote();
		if ($customer->getId()) {
			Mage::register('saferpaycw_external_checkout_login', true);
			Mage::getSingleton('customer/session')->setCustomerAsLoggedIn($customer);
			$quote->setCheckoutMethod(Mage_Checkout_Model_Type_Onepage::METHOD_CUSTOMER)->save();
			Mage::unregister('saferpaycw_external_checkout_login');
		}
