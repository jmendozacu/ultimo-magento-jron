...
class Customweb_SaferpayCw_Model_MotoObserver
{
	/**
			...
			if (strpos($payment->getMethodInstance()
				...
				if ($messageType == 'success') {
					...
					$controller->getResponse()
						->setRedirect(Mage::helper('adminhtml')->getUrl('*/motosaferpaycw/process', array(
	...
	public function beforeOrderSave(Varien_Event_Observer $observer)
	{
		try {
			...
			if (isset($_POST['payment']['method'])) {
				...
				if (strpos($paymentCode, 'saferpaycw') !== false) {
