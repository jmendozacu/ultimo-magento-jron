...
class Customweb_SaferpayCw_Model_InvoiceItems
{
	...
	public function getDiscountItem($item, $useBaseCurrency = false)
	{
		...
		if ($discountAmount != 0) {
			$sku = 'discount';
			$name = Mage::helper('SaferpayCw')->__("Discount");
			$taxRate = $discountTax;
			$quantity = 1;
			$amountIncludingTax = abs($discountAmount);
			$type = Customweb_Payment_Authorization_IInvoiceItem::TYPE_DISCOUNT;
	
			$discountItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku, $name, (double)$taxRate, (double)$amountIncludingTax, (double)$quantity, $type);
			return $discountItem;
		}
		return null;
	}
	...
	public function getShippingItem($invoice, $useBaseCurrency = false)
	{
		...
		if ($invoice->getShippingAmount() > 0) {
			...
			$shippingItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku, $shippingName, (double)$shippingTaxRate, (double)$shippingCostIncTax, (double)$quantity, $type);
			return $shippingItem;
		}
		return null;
	}
	...
	public function getFoomanSurchargeItem($order, $useBaseCurrency = false)
	{
		...
		if ($surchargeAmount != 0) {
			...
			$surchargeItem = new Customweb_Payment_Authorization_DefaultInvoiceItem($sku, $surchargeName, (double)$surchargeTaxRate, (double)$surchargeAmount, (double)$quantity, $type);
