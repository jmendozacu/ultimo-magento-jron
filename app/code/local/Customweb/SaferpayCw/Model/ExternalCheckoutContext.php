 ...
 * @version		1.3.251
 * 
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setContextId(int $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setState(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setFailedErrorMessage(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setCartUrl(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setDefaultCheckoutUrl(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setOrderAmountInDecimals(double $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setCurrencyCode(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setLanguageCode(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setCustomerEmailAddress(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setCustomerId(int $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setTransactionId(int $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setShippingMethodName(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setPaymentMethodMachineName(string $value)
 * @method string getCreatedOn()
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setCreatedOn(string $value)
 * @method string getUpdatedOn()
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setUpdatedOn(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setSecurityToken(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setSecurityTokenExpiryDate(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setAuthenticationSuccessUrl(string $value)
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setAuthenticationEmailAddress(string $value)
 * @method int getQuoteId()
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setQuoteId(int $value)
 * @method string getRegisterMethod()
 * @method Customweb_SaferpayCw_Model_ExternalCheckoutContext setRegisterMethod(string $value)
 * 
 */
class Customweb_SaferpayCw_Model_ExternalCheckoutContext extends Mage_Core_Model_Abstract implements Customweb_Payment_ExternalCheckout_IContext 
{
	/**
	 *
	...
	{
		$this->_init('saferpaycw/externalcheckoutcontext');
	}
	...
	protected function _afterLoad()
	{
		...
		$this->invoiceItems = Mage::helper('SaferpayCw')->unserialize($this->getData('invoice_items'));
		$this->shippingAddress = Mage::helper('SaferpayCw')->unserialize($this->getData('shipping_address'));
		$this->billingAddress = Mage::helper('SaferpayCw')->unserialize($this->getData('billing_address'));
		$this->providerData = Mage::helper('SaferpayCw')->unserialize($this->getData('provider_data'));
	...
	public function hasBasketChanged(Mage_Sales_Model_Quote $quote, Customweb_Core_Http_IRequest $request){
		...
		if (!isset($parameters['external-checkout-context-updated-on']) || $parameters['external-checkout-context-updated-on'] != $this->getUpdatedOn()
			...
			|| serialize($this->collectInvoiceItems($quote)) != serialize($this->getInvoiceItems())) {
			return true;
		} else {
			return false;
		}
	...
	private function collectInvoiceItems(Mage_Sales_Model_Quote $quote){
		return Mage::getModel('saferpaycw/invoiceItems')->getInvoiceItems($quote, $quote->getStore()->getStoreId(), $this->useBaseCurrency());
	}
	...
	public function setInvoiceItems($value) {
		if (is_string($value)) {
			$this->invoiceItems = Mage::helper('SaferpayCw')->unserialize($value);
		} else {
	...
	public function setShippingAddress($value) {
		if (is_string($value)) {
			$this->shippingAddress = Mage::helper('SaferpayCw')->unserialize($value);
		} else {
	...
	public function setBillingAddress($value) {
		if (is_string($value)) {
			$this->billingAddress = Mage::helper('SaferpayCw')->unserialize($value);
		} else {
	...
	public function setProviderData($value) {
		if (is_string($value)) {
			$this->providerData = Mage::helper('SaferpayCw')->unserialize($value);
		} else {
	...
	public function getPaymentMethod() {
		...
		if ($this->paymentMethod == null && !empty($paymentMethodMachineName)) {
			$this->paymentMethod = Mage::helper('payment')->getMethodInstance('saferpaycw_' . $paymentMethodMachineName);
		}
