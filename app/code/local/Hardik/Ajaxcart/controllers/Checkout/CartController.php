...
class Hardik_Ajaxcart_Checkout_CartController extends Mage_Checkout_CartController {
    ...
    public function addAction() {
        ...
        try {
            ...
            $product = $this->_initScpProduct($product->getId(), $this->getRequest()->getParam("super_attribute"));
            $params = $this->getRequest()->getParams();
     ...
     * @see Codekunst_Looks_CartController::_initLookProduct
     */
    protected function _initScpProduct($productId, $params)
    {
        if ($productId) {
            $parentProduct = Mage::getModel('catalog/product')
                ...
                ->load($productId);
            if ($parentProduct->getId() && $parentProduct->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) {
                if(Mage::helper('core')->isModuleEnabled('OrganicInternet_SimpleConfigurableProducts')) {
                    ...
                    $this->getRequest()->setParam("cpid", $product->getCpid());
                } else {
