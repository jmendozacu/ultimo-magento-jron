...
class Infortis_UltraMegamenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
				...
				{
								/** @var Mage_Catalog_Model_Category $x10 */
								if (!$x10->getIsActive()) {
												return '';
								}

                                // FORTUNEGLOBE FG: Only render 3 levels deep
                                if($x11 > 2) return;
								...
								if ($x14 && $x15) {
												$x31[] = $x15;
												$x32   = ' class="' . $x15 . ' menu-cat-' . $this->_getUrlKey($x10) . '"';
								}
								if ($x13) {
												$x31[] = 'first';
								}
				...
				{
					/* Customization FG BEGIN */
						/*
					    $cache = Mage::app()->getCache();
				        $uniqekey = "Navigation_Tree";
				        if( ! $data = $cache->load($uniqekey)){
							Mage::dispatchEvent('createNavigationTree');
				            $data = "Erstellt";
				            $cache->save($data, $uniqekey, array($uniqekey));
				        }
				        */
				    /* Customization FG END */
				...
				public function _getUrlKey($category)
				{
					if ($category instanceof Mage_Catalog_Model_Category) {
						...
						$urlKey = $this->_getCategoryInstance()
